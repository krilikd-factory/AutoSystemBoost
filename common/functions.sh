...
device_check() {
  ...
  for i in /system /vendor /odm /product /system_ext /mi_ext /my_product; do
...
mount_mirrors() {
  ...
  for i in /vendor /odm /product /system_ext /my_product $PARTITIONS; do
...
EXTRAPART=true
mkdir -p $MODPATH/tools/$MAGISK_VER
if $KSU || [ "$(find $MODPATH/tools -maxdepth 1 -type d -iname "*kitsune*")" ] || [ "$(find $MODPATH/tools -maxdepth 1 -type d -iname "*delta*")" ]; then
  EXTRAPART=true
elif ! $PARTOVER; then
  unset PARTITIONS
fi
...
for i in /system/vendor /vendor /mi_ext /my_product /system/system_ext /system_ext /mi_ext/etc /my_product/etc /my_product/vendor/etc /system/system_ext/etc /system_ext/etc /mi_ext/app /my_product/app /system/system_ext/app /system_ext/app /system/vendor/app /vendor/app /system/vendor/etc /vendor/etc /system/odm/etc /odm/etc /system/vendor/odm/etc /vendor/odm/etc /system/vendor/overlay /vendor/overlay; do
...
[ -d $MODPATH/bin/arm64-v8a ] && set_perm_recursive $MODPATH/bin/arm64-v8a 0 0 0755 0644
