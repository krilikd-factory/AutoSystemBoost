...
_asb_sed_do() { local mode="$1"; shift; if [ "$mode" = "gnu" ]; then asb_sed "$@"; else asb_sed "" "$@"; fi; }
asb_sed() {
  if [ -z "$ASB_SED_INPLACE_MODE" ]; then
    local t="${TMPDIR:-/dev/tmp}/.asb_sedtest.$$"
    echo x > "$t" 2>/dev/null
    if _asb_sed_do gnu 's/x/x/' "$t" >/dev/null 2>&1; then ASB_SED_INPLACE_MODE='gnu';
    elif _asb_sed_do bsd 's/x/x/' "$t" >/dev/null 2>&1; then ASB_SED_INPLACE_MODE='bsd';
    else ASB_SED_INPLACE_MODE='none'; fi
    rm -f "$t" 2>/dev/null
  fi
  if [ "$ASB_SED_INPLACE_MODE" = 'gnu' ]; then _asb_sed_do gnu "$@";
  elif [ "$ASB_SED_INPLACE_MODE" = 'bsd' ]; then _asb_sed_do bsd "$@";
  else sed "$@"; fi
}
set +x
set +v
...
asb_detect_compat() {
  ASB_MODEL_RAW="$(asb_prop_first ro.product.model ro.product.vendor.model ro.product.system.model)"
  ASB_DEVICE_RAW="$(asb_prop_first ro.product.device ro.product.vendor.device ro.vendor.product.device ro.build.product)"
  ASB_MANUFACTURER_RAW="$(asb_prop_first ro.product.manufacturer ro.product.vendor.manufacturer ro.vendor.product.manufacturer)"
  ASB_MODEL_L="$(echo "$ASB_MODEL_RAW" | tr '[:upper:]' '[:lower:]')"
  ASB_DEVICE_L="$(echo "$ASB_DEVICE_RAW" | tr '[:upper:]' '[:lower:]')"
  ASB_MANUFACTURER_L="$(echo "$ASB_MANUFACTURER_RAW" | tr '[:upper:]' '[:lower:]')"

  ASB_IS_ONEPLUS=false
  ASB_IS_OP15=false

  echo "$ASB_MANUFACTURER_L $ASB_MODEL_L $ASB_DEVICE_L" | grep -qi "oneplus" && ASB_IS_ONEPLUS=true

  case "$ASB_MODEL_L $ASB_DEVICE_L" in
    *"oneplus 15"*|*"oneplus15"*|*"op15"*|*"cph2745"*|*"op61fl1"*|*"pjz110"*|*"pkz110"*)
      ASB_IS_OP15=true ;;
  esac

  # Extra fallback for regional/product codenames seen on OP15 builds
  echo "$ASB_MODEL_L $ASB_DEVICE_L" | grep -Eqi '(^|[[:space:]])(cph2745|op61fl1|pjz110|pkz110)([[:space:]]|$)' && ASB_IS_OP15=true
}
