...
apply_dsp_compute_boost() {
  for n in /sys/kernel/debug/msm_subsys/cdsp /sys/kernel/debug/msm_subsys/adsp; do
    [ -e "$n" ] || continue
  done
  ...
  for d in /sys/devices/platform/soc/*cdsp*/power /sys/devices/platform/soc/*adsp*/power; do
    [ -d "$d" ] || continue
    [ -w "$d/control" ] && writef "$d/control" on 2>/dev/null || true
    [ -w "$d/autosuspend_delay_ms" ] && writef "$d/autosuspend_delay_ms" 2000 2>/dev/null || true
  done
}
