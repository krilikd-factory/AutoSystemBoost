...
# V39: governor.conf schema migration.
#
# Problem: /data/adb/modules/AutoSystemBoost/config/governor.conf persists
# across module installs. Older module versions shipped different defaults
# (V37: heavy_load_enter=5.4, V38: 8.0, V39+: 20.0). User stays on stale
# values forever unless they manually delete the file.
#
# Solution: schema_version sentinel. Each module ships an expected schema
# value. If user's config has missing/older schema marker, we backup their
# current config (with timestamp) and copy fresh from package. Their
# customizations are preserved as recoverable backup files.
#
# This runs ONCE per schema bump. After successful migration, schema marker
# is updated. Next boot sees current marker and skips entirely.
#
# Bump ASB_CONFIG_SCHEMA whenever defaults change in a way that would silently
# disadvantage users still running old persisted values.
asb_migrate_governor_conf() {
  local _expected_schema=10
  ...
  # If user's config doesn't exist at all, nothing to migrate
  [ -f "$_user_conf" ] || return 0

  # Read current schema marker; default to 0 if absent (predates marker system)
  ...
  # If already on current schema, skip
  ...
  # Backup current config with timestamp so user customizations are recoverable
  ...
  # The shipped governor.conf at $MODDIR/config/governor.conf is what we want.
  # When module is freshly installed, KSU/Magisk copies the entire $MODPATH to
  # $MODDIR. So $MODDIR/config/governor.conf IS the shipped one — but it was
  # already overwritten by Magisk install. Wait — actually no. If the user's
  # /data/adb/modules/AutoSystemBoost/ existed already, Magisk merges (some
  # files persist, some replaced — depends on what zip Magisk repackaged).
  # Most reliable: keep a sealed copy of the shipped default in module package
  # at config/governor.conf.shipped that's never modified by the user.
  if [ -f "$_conf_dir/governor.conf.shipped" ]; then
    ...
    asb_log "config_migrate: (this is expected on schema=0 -> 10 first migration)"
    return 1
  fi
...
apply_camera_runtime() {
  ...
  # V39: OnePlus camera feature enables. ro.vendor.* are read-only by setprop
  # contract — can only be modified via resetprop (Magisk/KSU-provided). Falls
  # back gracefully if resetprop missing.
