...
[ "$(lk_bt_lifecycle_kind '1.0 A2dpService: device disconnected')" = "disconnect" ] || fail "disconnect classification"
[ "$(lk_bt_lifecycle_kind '1.0 AdapterService: connected state=2')" = "connect" ] || fail "connect classification"
...
lk_bt_lifecycle_record '1787300000.100 AdapterService: device AA:BB:CC:DD:EE:FF connected PixelBuds'
lk_bt_lifecycle_record '1787300001.100 A2dpService: device AA:BB:CC:DD:EE:FF disconnected PixelBuds'
...
awk -F'\t' '!/^#/ { if (NF != 4 || $3 !~ /^(connect|disconnect|reconnect_literal)$/) exit 1 }' "$LK_BT_RECONNECT_EVENTS" || fail "event TSV shape"
...
grep -Fq 'event=reconnect_literal only when stack log writes reconnect' "$COMMON" || fail "literal reconnect limitation missing"
