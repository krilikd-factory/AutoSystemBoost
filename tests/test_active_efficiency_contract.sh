...
[ "$(get status "$TMP/active_efficiency.env")" = "generic" ] || fail "unknown SoC must be no-op"
[ "$(get budget_moderate_bonus_pct "$TMP/active_efficiency.env")" = "0" ] || fail "unknown SoC bonus must be zero"
