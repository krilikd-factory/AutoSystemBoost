#!/system/bin/sh
  ...
  _last_wifi_check=0
  _drift_streak=0
  # V39r2: lease-based reassert after profile change / screen state change.
  # Observed V39r2 battery log showed scaling_max_freq drifted to 1228/1382 MHz
  ...
  while true; do
    ...
    if [ "$_now" != "$_last_profile" ]; then
      ...
      # V39r2: open lease window — poll rapidly for 60s to re-assert caps
      ...
      if [ "$_cur_screen" != "$_last_screen" ]; then
        ...
        # V39r2: screen transitions are when vendor perf_boost most often
        # raises scaling_max_freq. Open lease window to catch the drift.
        _lease_remaining=4
      fi
      # V39r2: direction-aware cap drift check — only fight vendor if it
    ...
    if [ $_need -eq 1 ]; then
      case "$_reason" in
        ...
        cap-drift-up-p0|cap-drift-up-p6)
          # V39r2: cap drift isn't a slow WALT drift — it's vendor writing
      ...
      if [ "$ASB_GOV_ENABLED" = "1" ] && asb_governor_running; then
        if [ "$_reason" = "profile-change" ]; then
          ...
          # V39r2: vendor raised caps above profile. Reassert via shell
