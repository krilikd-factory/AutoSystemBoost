...
asb_governor_set_profile() {
  ...
  asb_governor_running || return 0
  _gsp_reply="$("$ASB_GOV" "profile:$ASB_PROFILE" 2>/dev/null)"
  case "$_gsp_reply" in
    ok|ok:nochange) asb_log "governor_set_profile: $ASB_PROFILE ($_gsp_reply)" ;;
    *) asb_log "governor_set_profile: WARNING reply=[$_gsp_reply] profile=$ASB_PROFILE" ;;
  esac
}
