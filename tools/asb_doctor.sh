#!/system/bin/sh
# ASB Doctor — one-shot module health check
# Usage: su -c sh /data/adb/modules/AutoSystemBoost/tools/asb_doctor.sh
...
benchmark_isolation_status() {
  ...
  grep -q 'benchmark+thermal detected but burst preserved' "$_src" || return 1
...
elif [ -f "$MODDIR/src/asb_governor.c" ]; then
  warn "asb_governor binary not present in source tree/archive"
else
...
for path in \
  ...
  /sys/class/kgsl/kgsl-3d0/devfreq/min_freq; do
  if [ -e "$path" ]; then
    if [ -w "$path" ]; then
      ok "$path writable"
    else
      warn "$path exists but not writable"
    fi
  else
    warn "$path not found (device topology may differ)"
  fi
done
...
if benchmark_isolation_status; then
  ...
  warn "cannot verify benchmark isolation (source missing)"
else
