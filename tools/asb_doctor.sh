...
#   or:  sh tools/asb_doctor.sh   (from source tree)
...
# --- sysfs paths (only when installed) ---
...
# --- Conflict detection (only when installed) ---
if [ $IS_INSTALLED -eq 1 ]; then
  echo
  echo "🔍 Conflict Scan"
  CONFLICTS=""
...
# V37: type-based zone discovery, never hardcode zone IDs
...
if [ -d "$THERMAL_BASE" ]; then
  ...
  for sensor in socd cpu-1-1-0 cpu-0-5-0 cpullc-0-0 shell_front shell_frame shell_back sys-therm-6 board_temp; do
    ...
    if [ -n "$ZONE" ]; then
      ...
      [ -z "$RAW" ] && RAW=0
      # Heuristic: >200 = millidegrees, else raw celsius
