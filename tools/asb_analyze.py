...
def parse_log(path):
    ...
    for i, line in enumerate(lines):
        if 'session_start' in line or 'session reset' in line:
            last_session_idx = i
    ...
    for line in session_lines:
        if 'intent: classified as' in line:
            ...
            if m:
                evt = m.group(1).strip()
                if evt not in result['self_tune_events']:
                    result['self_tune_events'].append(evt)
            ...
            if m:
                evt = m.group(1).strip()
                if evt not in result['feedback_events']:
                    result['feedback_events'].append(evt)
...
def print_report(sessions):
    ...
    for s in sessions:
        ...
        if explain_lines:
            ...
            # V29: Explain confidence
            conf_score = 3  # start at high
            if total_all == 0: conf_score -= 2  # no FSM events
            if s.get('restarts', 0) > 0: conf_score -= 1  # governor restarted
            if s.get('end', '') not in ('profile_change', 'idle_boundary', 'shutdown', ''): conf_score -= 1
            conf_label = {3: "high", 2: "medium", 1: "low", 0: "low"}.get(max(0, conf_score), "low")

            print(f"\n  ┌─ Explain (confidence: {conf_label}) ─")
            if total_all == 0:
                print(f"  │  ⚠ Timeline incomplete — explain may include cross-session events")
            for el in unique:
                print(f"  │  {el}")
