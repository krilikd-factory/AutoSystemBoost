...
def session_trust(s):
    # V34: use governor's bat_trust field directly if available
    bt = s.get('bat_trust', -1)
    if bt >= 0:
        return {0: 'dirty', 1: 'partial', 2: 'clean'}.get(bt, 'clean')
    # Fallback for older sessions without bat_trust field
    dur = s.get('dur', 0)
    iq  = s.get('idle_q', -1)
    bd  = s.get('bat_deep', 0)
    bw  = s.get('bat_wake', 0)
    ...
    if prof == 'battery' and dur > 3600 and bd < 120 and iq < 0 and bw <= 2:
        return 'partial'
    # V34: match governor's penalized iq + wph checks
    if prof == 'battery' and dur >= 300:
        bt_total = s.get('bat_deep', 0) + s.get('bat_light', 0) + s.get('bat_mod', 0)
        if bt_total > 0:
            p_iq = int(s.get('bat_deep', 0) * 100 / bt_total)
            wp = max(0, (bw - 2)) * 5 if bw > 2 else 0
            p_iq = max(0, p_iq - wp)
            if p_iq < 20:
                return 'dirty'
...
def generate_report(sessions):
    ...
    def classify_headroom(s):
        # V30: prefer C-side limiter from v6 history
        c_limiter = s.get('limiter', 'none')
        hr_n = s.get('hr_n', 0)
        ...
        # Use C-side if available (v6+)
        if c_limiter and c_limiter != 'none':
            return c_limiter, early, conf
        # Fallback: Python classification for v5 records
        prof = s.get('profile', '')
        thermal_hot = mt >= (90 if prof == 'performance' else 80)
        if ce >= 70 and b70_pct < 10 and hr_min >= 70:
            return "reachable", early, conf
        unreach = s.get('unreachable', 0)
        if ce < 40 and unreach >= 10 and not thermal_hot and b50_pct < 10:
    ...
    # V30-r3: Session signatures and confidence (v7+ records)
    v7_sessions = [s for s in sessions if s.get('v', 0) >= 7 and s.get('sig')]
    if v7_sessions:
        sig_counts = {}
        conf_counts = {'high': 0, 'medium': 0, 'low': 0}
        anomaly_counts = {}
        mt_counts = {}
        for s in v7_sessions:
            sig = s.get('sig', 'mixed')
            sig_counts[sig] = sig_counts.get(sig, 0) + 1
            c = s.get('conf', 'low')
            if c in conf_counts: conf_counts[c] += 1
            a = s.get('anomaly', 'none')
            if a != 'none':
                anomaly_counts[a] = anomaly_counts.get(a, 0) + 1
            mt = s.get('mid_tune', 'none')
            if mt != 'none':
                mt_counts[mt] = mt_counts.get(mt, 0) + 1

        lines.append(f"\n## 🏷️ Session Signatures (v7+, {len(v7_sessions)} sessions)\n")
        sig_parts = [f"**{k}**={v}" for k, v in sorted(sig_counts.items(), key=lambda x: -x[1])]
        lines.append(f"- Signatures: {', '.join(sig_parts)}")
        conf_parts = [f"{k}={v}" for k, v in conf_counts.items() if v > 0]
        lines.append(f"- Confidence: {', '.join(conf_parts)}")
        if anomaly_counts:
            anom_parts = [f"**{k}**={v}" for k, v in anomaly_counts.items()]
            lines.append(f"- ⚠️ Anomalies: {', '.join(anom_parts)}")
        if mt_counts:
