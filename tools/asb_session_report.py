...
def session_trust(s):
    """
    Session data quality flag: clean / partial / dirty.
    Used to avoid learning from incomplete telemetry.
    """
    dur = s.get('dur', 0)
    iq  = s.get('idle_q', -1)
    bd  = s.get('bat_deep', 0)
    ...
    if dur < 120:
        return 'dirty'  # too short for meaningful data
    if prof == 'battery' and dur > 3600 and bd < 120 and iq < 0 and bw <= 2:
        return 'partial'  # telemetry incomplete (pre-flush build)
    if iq == -1 and prof == 'battery' and dur > 300:
        return 'partial'  # idle_q not computed
    return 'clean'
...
def generate_report(sessions):
    ...
    # ── Runtime trust ───────────────────────────────────────────────────────
    trust_counts = {'clean': 0, 'partial': 0, 'dirty': 0}
    for s in sessions:
        t = session_trust(s)
        trust_counts[t] = trust_counts.get(t, 0) + 1
    if trust_counts['partial'] > 0 or trust_counts['dirty'] > 0:
        lines.append("## 🔍 Data Quality\n")
        lines.append(f"- **clean**: {trust_counts['clean']} sessions")
        if trust_counts['partial'] > 0:
            lines.append(f"- **partial**: {trust_counts['partial']} sessions (incomplete telemetry — excluded from learning)")
        if trust_counts['dirty'] > 0:
            lines.append(f"- **dirty**: {trust_counts['dirty']} sessions (too short — not scored)")
        lines.append("")
