...
lk_verify_caps() {
...

  {
    ...
    for pd in /sys/devices/system/cpu/cpufreq/policy*; do
      ...
      # V39r2: source classification mirrors the governor's cap_source_classify()
      # so the shell verdict stays in sync with the C-side canonical answer.
      if [ -z "$_expect" ] || [ -z "$_smax" ]; then
        _status="unset"
        _source="-"
      elif [ "$_smax" = "$_expect" ] && [ -n "$_asb_declared" ] && [ "$_asb_declared" = "$_expect" ]; then
        _status="ok"
        _source="asb"
      else
        _diff=$((_smax - _expect))
        if [ -n "$_asb_declared" ] && [ "$_asb_declared" != "0" ] && [ "$_smax" = "$_asb_declared" ] && [ "$_asb_declared" != "$_expect" ]; then
        ...
        elif [ -n "$_asb_declared" ] && [ "$_asb_declared" != "0" ] && [ "$_smax" -gt "$_asb_declared" ] 2>/dev/null; then
          _status="DESYNC_vendor_raised (asb_declared=$_asb_declared actual=$_smax over=+$((_smax - _asb_declared)))"
          _source="vendor_raised"
        elif [ -n "$_asb_declared" ] && [ "$_asb_declared" != "0" ] && [ "$_smax" -lt "$_asb_declared" ] 2>/dev/null \
...
lk_emit_cap_source_summary() {
  # V39r2: post-capture summary of how much time each cap_source state held.
