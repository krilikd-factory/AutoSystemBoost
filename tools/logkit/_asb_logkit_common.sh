...
lk_verify_caps() {
  # V39: multi-source cap verification.
  #
  # V38 cap_verify only compared `scaling_max_freq` (what vendor-visible layer
  # reports) to the profile's expected cap. Real-device V38 logs showed 100%
  # DESYNC rates, which was confusing because ASB was doing its job correctly
  # — the mismatch was caused by the vendor thermal HAL clamping CPU caps
  # below what ASB requested. cap_verify_v1 couldn't distinguish "ASB bug"
  # from "vendor overriding ASB".
  #
  # V39 cap_verify probes all three layers:
  #   - scaling_max_freq       — vendor-visible effective limit (lowest of all)
  #   - msm_performance_interface — what ASB wrote last via kernel interface
  #   - cpufreq/policyN/cpuinfo_max_freq — hardware ceiling (static)
  # And classifies the source of DESYNC when it happens.
  ...
  # Read ASB's declared caps from status JSON (source of truth for "what ASB wants")
  _j=$(lk_status_json)
  ...
  {
    echo "# cap_verify v2 at $(date -u '+%Y-%m-%dT%H:%M:%SZ')  profile=$_profile"
    ...
    for pd in /sys/devices/system/cpu/cpufreq/policy*; do
      ...
      # V39: source-aware status classification.
      if [ -z "$_expect" ] || [ -z "$_smax" ]; then
        _status="unset"
        _source="-"
      elif [ "$_smax" = "$_expect" ]; then
        _status="ok"
        _source="asb"
      else
        # There's a DESYNC. Figure out who's at fault.
        _diff=$((_smax - _expect))
        if [ -n "$_asb_declared" ] && [ "$_asb_declared" != "0" ] && [ "$_smax" = "$_asb_declared" ]; then
          # scaling_max matches what ASB declared via its own interface, but
          # that's different from the profile file. ASB has applied a dynamic
          # override (SUSTAINED clamp, thermal overlay, etc.) — not a bug.
          ...
          # Vendor HAL raised the cap above what ASB/profile wants. Common on OP15.
          _status="DESYNC_vendor_raised (profile=$_expect actual=$_smax diff=+$_diff over-spec)"
          _source="vendor_raised"
        elif [ -n "$_cmax" ] && [ "$_smax" -lt "$_cmax" ] 2>/dev/null; then
          # scaling_max is below hardware ceiling — likely thermal/vendor clamp.
          _status="DESYNC_vendor_clamp (profile=$_expect actual=$_smax clamp_depth=$((_cmax - _smax)))"
          _source="vendor_clamp"
        else
          _status="DESYNC_unknown (profile=$_expect actual=$_smax diff=$_diff)"
          _source="unknown"
        fi
      fi
      echo "policy${_p} (${_label}) cpus[first]=${_rel} actual_max=${_smax} hw_ceiling=${_cmax:-?} profile_expected=${_expect:-unset} asb_declared=${_asb_declared:-?} source=${_source} -> $_status"
    done
