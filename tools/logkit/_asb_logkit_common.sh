...
lk_grep_governor_log_events() {
  ...
  grep -E "enter_sustained|exit_sustained"      "$LK_OUT_DIR/governor.log.session" > "$LK_OUT_DIR/events_sustained.txt"     2>/dev/null
  grep -E "thermal_cpu_switch|thermal_cpu_choice|thermal_summary" "$LK_OUT_DIR/governor.log.session" > "$LK_OUT_DIR/events_thermal_source.txt" 2>/dev/null
