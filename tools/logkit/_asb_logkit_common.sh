...
lk_status_json() {
  # Emit current ASB status as a single JSON line. `asb status` is preferred
  # (uses Unix socket for atomic snapshot); fall back to runtime file.
  if lk_have asb; then
...
lk_grep_governor_log_events() {
  ...
  # per-category files. Makes diagnosis fast without scrolling governor.log.
  if [ -f "$LK_OUT_DIR/governor.log" ]; then
    grep -E "enter_sustained|exit_sustained"      "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_sustained.txt"     2>/dev/null
    grep -E "thermal_cpu_switch|thermal_cpu_choice|thermal_summary" "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_thermal_source.txt" 2>/dev/null
    grep -E "self_tune|mid_tune|auto_degrade"     "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_tuning.txt"        2>/dev/null
    grep -E "pstats: battery trust=|bat_trust"    "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_battery_trust.txt" 2>/dev/null
    grep -E "screen_aware_caps|apply_cpufreq_caps|reconcile" "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_cap_apply.txt" 2>/dev/null
    grep -E "intent=|session_end|session_start"   "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_session.txt"       2>/dev/null
    grep -E "headroom_valid=0|stuck_100|headroom_invalid" "$LK_OUT_DIR/governor.log" > "$LK_OUT_DIR/events_headroom.txt" 2>/dev/null
  fi
}
