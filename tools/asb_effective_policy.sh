...
printf '"energy_policy":{"shadow_mode":"%s","thermal_budget_enabled":"%s","thermal_budget_trim_pct":"%s","thermal_budget_base_trim_pct":"%s","thermal_budget_envelope_bonus_pct":"%s","thermal_budget_stage":"%s","thermal_budget_reason":"%s","thermal_budget_dwell_s":"%s"},' \
  "$(_json "$(_state shadow_mode)")" "$(_json "$(_state thermal_budget_enabled)")" "$(_json "$(_state thermal_budget_trim_pct)")" "$(_json "$(_state thermal_budget_base_trim_pct)")" "$(_json "$(_state thermal_budget_envelope_bonus_pct)")" "$(_json "$(_state thermal_budget_stage)")" "$(_json "$(_state thermal_budget_reason)")" "$(_json "$(_state thermal_budget_dwell_s)")"
printf '"active_efficiency":{"active":"%s","tier":"%s","reason":"%s","gpu_idle_bonus_pct":"%s","bg_uclamp_moderate_delta":"%s","bg_uclamp_severe_delta":"%s"},' \
  "$(_json "$(_state active_efficiency_active)")" "$(_json "$(_state active_efficiency_tier)")" "$(_json "$(_state active_efficiency_reason)")" "$(_json "$(_state active_efficiency_gpu_idle_bonus_pct)")" "$(_json "$(_state active_efficiency_bg_uclamp_moderate_delta)")" "$(_json "$(_state active_efficiency_bg_uclamp_severe_delta)")"
