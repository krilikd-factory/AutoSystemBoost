 ...
 *
 * This helper compares what ASB last wrote (fsm->current_caps.cpu_max[])
 * against what sysfs currently reports (scaling_max_freq) and returns
 * a string telling downstream analysis who is in control of the cap
 * right now. "asb" means our write stuck; "vendor_clamp" / "vendor_raised"
 * means the vendor layer overrode us.
 */
static inline const char *cap_source_classify(int asb_declared,
                                              int actual_sysfs,
                                              int hw_ceiling) {
    if (asb_declared <= 0 || actual_sysfs <= 0) return "unknown";
    if (asb_declared == actual_sysfs)           return "asb";
    if (actual_sysfs > asb_declared)            return "vendor_raised";
    /* actual < declared */
    if (hw_ceiling > 0 && actual_sysfs < hw_ceiling) return "vendor_clamp";
    return "mismatch";
}
                               ...
                               char *out, int outlen)
{
    ...
    const char *cap_src_p0 = cap_source_classify(fsm->current_caps.cpu_max[0],
                                                 real_max_p0, hw_ceil_p0);
    const char *cap_src_p6 = cap_source_classify(fsm->current_caps.cpu_max[1],
