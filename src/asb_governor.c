...
static void pstats_save_one(const char *path, const asb_persistent_stats_t *ps) {
    ...
    snprintf(buf, sizeof(buf),
        "{\"count\":%d,\"t2s\":%.1f,\"t2th\":%.1f,\"temp\":%.1f,\"gap\":%.0f,\"eff\":%.1f,\"deg\":%d,\"hot\":%d,\"deg_age\":%.1f,\"btcd\":%d}",
            ps->session_count, ps->avg_time_to_first_sus,
            ps->avg_time_to_first_thermal, ps->avg_max_temp,
            ps->avg_gap_p0, ps->avg_efficiency, ps->degrade_count,
            ps->hot_fail_count, ps->avg_degrade_age,
            ps->bat_tune_cooldown);
    ...
    const char *limiter, int reach, const char *bat_reason)
{
    if (fsm->profile_idx == PROFILE_PERFORMANCE || fsm->profile_idx == PROFILE_BALANCED) {
        ...
        if (strcmp(limiter, "thermal") == 0) return "thermal_limited";
    ...
    if (fsm->profile_idx == PROFILE_BATTERY) {
        if (strcmp(bat_reason, "none") == 0) return "clean_sleep";
...
static const char *classify_anomaly(
    const asb_fsm_t *fsm, long dur, int idle_quality)
{
    if (fsm->ses_max_temp >= 100) return "extreme_temp";
...
static void session_history_append_ex(const asb_fsm_t *fsm, const char *reason) {
    ...
    if (fsm->profile_idx == PROFILE_BATTERY) {
         ...
         * performance: 90°C (high thermal headroom expected)
         * balanced/other: 80°C (lower threshold matches Python fallback) */
        int thermal_hot = 0;
    ...
    const char *sig  = classify_signature(fsm, sus_pct, limiter, reach, bat_reason);
    const char *anomaly = classify_anomaly(fsm, dur, idle_quality);
...
int main(int argc, char **argv) {
    ...
    metrics_read_all(&metrics, 1);  /* startup: always read headroom */
    ...
    while (g_running) {
        ...
        if (need_metrics) {
            /* V29-r13: skip headroom sysfs read in battery screen-off */
            int need_hr = 1;
            if (fsm.profile_idx == PROFILE_BATTERY &&
                (!metrics.misc.screen_on || fsm.state < ASB_STATE_MODERATE))
                need_hr = 0;
            metrics_read_all(&metrics, need_hr);
