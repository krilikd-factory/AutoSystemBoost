...
int main(int argc, char **argv) {
    ...
    /* ─── Reassert tracker ──────────────────────────────────── */
    /* При HEAVY/GAMING без thermal governor периодически переподтверждает
     * caps через msm_performance. Это борется с vendor perf HAL который
     * может откатить наши значения.
     * Интервал: GAMING=6с, HEAVY=12с. При thermal=1 — не reassert.
     */
    time_t g_last_reassert = 0;
#define REASSERT_GAMING_S  6   /* переподтверждение в GAMING */
#define REASSERT_HEAVY_S   12  /* переподтверждение в HEAVY  */
    ...
    while (g_running) {
        ...
        if (need_metrics) {
            ...
            /* Vendor perf HAL может откатить scaling_max_freq через
             * несколько секунд после нашей записи. Решение: периодически
             * переподтверждаем через msm_performance.
             * Условия reassert:
             *   1. State >= HEAVY (только для высоких нагрузок)
             *   2. thermal == 0 (при thermal — уважаем thermal governor)
             *   3. Прошло достаточно времени с последнего reassert
             */
            if (fsm.state >= ASB_STATE_HEAVY && !fsm.thermal_cap) {
                int reassert_interval = (fsm.state == ASB_STATE_GAMING)
                                        ? REASSERT_GAMING_S : REASSERT_HEAVY_S;
                time_t now = time(NULL);
                if (now - g_last_reassert >= reassert_interval) {
                    /* Используем msm_performance напрямую — только этот канал
                     * надёжно пробивает vendor perf HAL override */
                ...
                /* Не в high state — сбрасываем счётчик чтобы при входе
                 * в HEAVY/GAMING reassert сработал немедленно */
                if (fsm.state < ASB_STATE_HEAVY)
                    g_last_reassert = 0;
            }
        }
