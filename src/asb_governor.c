...
int main(int argc, char **argv) {
    ...
    write_state(&fsm, &metrics, cur_pred);
    {
        ...
        {
            char _zt_path[128], _zt_type[64] = "unknown";
            snprintf(_zt_path, sizeof(_zt_path),
                     "/sys/class/thermal/thermal_zone%d/type", g_thermal_cpu_zone);
            FILE *_ztf = fopen(_zt_path, "r");
            if (_ztf) {
                if (fgets(_zt_type, sizeof(_zt_type), _ztf)) {
                    char *_nl = strchr(_zt_type, '\n');
                    if (_nl) *_nl = '\0';
                }
                fclose(_ztf);
            }
            asb_log("diag: thermal_cpu_zone_type=%s", _zt_type);
        }
