...
/* V39r5: shell_overridden_up tracker for non-perf profiles.
                               ...
                               char *out, int outlen)
{
    ...
    /* V39r5: classifier gets runtime_declared from msm_performance read (m->therm.perf_cap_*),
...
int main(int argc, char **argv) {
    ...
    while (g_running) {
        ...
        if (need_metrics) {
            ...
            /* V39r8: thermal_pwrlevel monitoring with three caution gates.
             *
             ...
             *   4. In LIGHT_IDLE/MODERATE: read every Nth tick (configurable)
             *   5. In HEAVY/SUSTAINED/GAMING: read every tick
             */
            {
                ...
                /* V39r8: thread thermal_pwrlevel into thermal struct for FSM use.
            ...
            /* V39r5: shell_overridden_up watchdog
             * Anti-clamp above handles vendor DOWN-clamps (gap > 0). Battery profile
             * probe showed intermittent vendor UP-raising in MODERATE (gap_p0=-265920,
             * cap_source=vendor_raised) — sysfs scaling_max_freq goes ABOVE what the
             * FSM wrote for ~7 samples, leaking battery-profile caps toward balanced.
             *
             * Trigger: non-perf profile, actual > desired by >100 MHz, for 2+ consecutive
             * ticks. Rewrite scaling_max_freq once per 4s. Separate cadence from anti-clamp
             * to avoid thrash. Cap at 5 reassert/minute to prevent write war if vendor
             * layer persistently overrides (in that case we let it win, it's above our target
             * for a reason — thermal headroom or perf hint). */
            if (fsm.profile_idx != PROFILE_PERFORMANCE &&
                ...
                int any_streak = (g_leak_streak_p0 >= 2 || g_leak_streak_p1 >= 2);
                time_t leak_now = time(NULL);
                int cooldown_ok = (leak_now - g_last_leak_reassert >= 4);

                /* Rate-limit: if we've reasserted 5+ times in the last minute without
                 * winning, vendor layer is persistent — stop fighting. */
                static time_t g_leak_minute_start = 0;
                if (leak_now - g_leak_minute_start >= 60) {
                    g_leak_minute_start = leak_now;
                    g_leak_reassert_count = 0;
                }
                int rate_ok = (g_leak_reassert_count < 5);

                if (any_streak && cooldown_ok && rate_ok) {
                    if (leak0 && want_p0 > 0)
                        sysfs_write_int(cpu_policy_path(0, "scaling_max_freq"), want_p0);
                    if (leak1 && want_p1 > 0)
                        sysfs_write_int(cpu_policy_path(1, "scaling_max_freq"), want_p1);
                    g_last_leak_reassert = leak_now;
                    g_leak_reassert_count++;
                    if (g_asb_cfg.log_level >= 1) {
                        asb_log("leak_reassert[%s/%s]: p0=%d->%d (streak=%d) p1=%d->%d (streak=%d) count=%d/5",
                                asb_state_names[fsm.state],
                                (fsm.profile_idx == PROFILE_BATTERY) ? "battery" : "balanced",
                                actual_p0, want_p0, g_leak_streak_p0,
                                actual_p1, want_p1, g_leak_streak_p1,
                                g_leak_reassert_count);
                    }
                    g_leak_streak_p0 = 0;
                    g_leak_streak_p1 = 0;
             ...
             * V39r2: context-aware threshold. The V39r1 battery_mixed log showed
