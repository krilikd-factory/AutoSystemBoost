...
int main(int argc, char **argv) {
    ...
    while (g_running) {
        ...
        if (need_metrics) {
             ...
             * If wake_cycles >= 5 and session > 5min, system is too noisy
             * for useful governor work. Go ultra-light. */
            ...
            if (g_storm_shield_active && fsm_profile_is_battery
                ...
                if (fsm.bat_wake_cycles == g_shield_last_wakes) {
                    ...
                    if (g_shield_calm_ticks >= 60) {  /* ~10min at 10s deep ticks */
                        storm_shield_reset();
                        session_plan_build(&fsm, 0);
                        asb_log("storm_shield: exited (noise calmed for ~10min, wakes=%d)",
                                fsm.bat_wake_cycles);
                    }
                } else {
                    g_shield_last_wakes = fsm.bat_wake_cycles;
             ...
             * Runs every ~5min (60 ticks) to avoid re-introducing jitter. */
            if (fsm.clamp_hold && metrics.misc.screen_on) {
                if (++g_clamp_probe_skip >= 60) {
                    g_clamp_probe_skip = 0;
                    ...
                    if (max_gap < 500000) {
                        ...
                        session_plan_build(&fsm, metrics.misc.screen_on);
                        session_plan_apply_prearm(&fsm);
                        asb_log("clamp_probe: vendor clamp lifted (gap=%dkHz/%dkHz), plan rebuilt",
                                gap0 / 1000, gap1 / 1000);
                    } else {
