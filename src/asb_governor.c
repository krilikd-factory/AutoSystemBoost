...
static int battery_session_trust(const asb_fsm_t *fsm) {
    ...
    if (fsm->ses_intent == INTENT_SLEEP_IDLE)
        return BAT_TRUST_PARTIAL;
    /* V33: noisy battery sessions should not contaminate learning.
     * idle_q=0 means no settle at all; high wakes = phone never rested. */
    int iq = 0;
    if (bat_total > 0)
