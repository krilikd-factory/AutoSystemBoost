...
int main(int argc, char **argv) {
    ...
    /* Диагностика при старте — пишем в лог что нашли */
    {
        ...
        asb_log("diag: cfg heavy_gpu=%d heavy_load=%.1f gaming_gpu=%d sustained_gpu=%d sustained_load=%.1f sustained_temp=%d sustained_lvl=%.2f dwell=%d/%d/%d reassert=%d/%d boost_only=%d throttle_temp=%d",
            ...
            if (changed || force_write) {
                ...
                if (fsm.state_changed) {
                    ...
                    if (fsm.state == ASB_STATE_SUSTAINED) {
                        asb_log("enter_sustained: temp>=%d thermal_cap=%d",
                    ...
                    if (fsm.prev_state == ASB_STATE_SUSTAINED) {
                        const char *reason = (metrics.therm.cpu_max_c < g_asb_cfg.sustained_temp_enter)
                                             ? "temp_dropped"
                                             : "no_longer_heavy";
                        asb_log("exit_sustained: %s t=%d°C -> %s",
