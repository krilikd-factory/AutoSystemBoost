                               ...
                               char *out, int outlen)
{
    ...
    snprintf(out, outlen,
        ...
        "\"last_sustained_reason\":\"%s\",\"highload_mode\":\"%s\",\"ses_gaming\":%d,\"ses_sustained\":%d,\"ses_thermal\":%d,\"ses_unreachable\":%d,\"ses_t_heavy\":%ld,\"ses_t_gaming\":%ld,\"ses_t_sustained\":%ld,\"ses_avg_gap_p0\":%d,\"ses_max_gap_p0\":%d,\"ses_max_temp\":%d,\"ses_auto_degraded\":%d}",
        asb_state_names[fsm->state],
        profile_names[fsm->profile_idx],
...
int main(int argc, char **argv) {
    ...
    while (g_running) {
        ...
        for (int i = 0; i < nev; i++) {
            ...
            /* ── Control socket ──────────────────────────────── */
                ...
                if (strncmp(cmd, "profile:", 8) == 0) {
                    ...
                    asb_config_load_file(CONFIG_FILE, &g_asb_cfg);
                    asb_config_apply_highload_mode(&g_asb_cfg);
                    fsm_session_reset(&fsm); /* сброс телеметрии при reload */
                    if (new_idx != fsm.profile_idx) {
                        fsm.profile_idx = new_idx;
                        force_write = 1;
                        need_metrics = 1;
        ...
        if (need_metrics) {
            ...
            /* AUTO degrade check (V22-r13): burst→stable при недостижимых gaming caps */
            if (!fsm.ses_auto_degraded) {
                        ...
                        fsm.ses_auto_degraded))
                {
                    ...
                    asb_log("auto: degraded burst->stable avg_gap=%d sus=%d gaming=%d",
                            avg_gap, fsm.ses_sustained_entries, fsm.ses_gaming_entries);
                }
            }
