    ...
    const asb_fsm_t *fsm, long dur, int idle_quality)
{
    ...
    /* V30: battery failed_settle -- long session with terrible idle quality */
    if (fsm->profile_idx == PROFILE_BATTERY && dur >= 1800
        && idle_quality >= 0 && idle_quality < 20
...
static void session_history_append_ex(const asb_fsm_t *fsm, const char *reason) {
    ...
    /* V30: skip boundary carry-over sessions (short hot profile switches)
     * These pollute aggregate with thermal_limited / extreme_temp from carry-over heat */
