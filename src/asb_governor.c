...
#define CONFIG_FILE     "/data/adb/modules/AutoSystemBoost/governor.conf"
...
int main(int argc, char **argv) {
    ...
    /* Диагностика при старте — пишем в лог что нашли */
    {
        ...
        asb_log("diag: cfg heavy_gpu=%d heavy_load=%.1f gaming_gpu=%d sustained_gpu=%d sustained_load=%.1f sustained_temp=%d dwell=%d/%d/%d reassert=%d/%d boost_only=%d throttle_temp=%d",
    ...
    while (g_running) {
        ...
        if (need_metrics) {
            ...
            g_msm_boost_active = (fsm.state == ASB_STATE_HEAVY || fsm.state == ASB_STATE_GAMING) && !fsm.thermal_cap;
