 ...
 *
 * V39r2 refinement (ChatGPT P1): richer state set.
 * The classifier now takes both the profile's nominal cap AND ASB's
 * current runtime declared cap. This separates:
 *   - "asb"             — profile cap == runtime == actual  (clean state)
 *   - "asb_dynamic"     — runtime != profile, but actual == runtime  (our clamp stuck)
 *   - "thermal_overlay" — actual == profile but runtime is different (overlay active)
 *   - "vendor_clamp"    — actual < runtime and < hw ceiling  (vendor HAL clamped)
 *   - "vendor_raised"   — actual > runtime                   (vendor HAL raised)
 *   - "policy_unknown"  — sysfs read failed or nonsensical
 *   - "mismatch"        — none of the above matched (edge case, investigate)
 */
static inline const char *cap_source_classify(int profile_cap,
    ...
    if (profile_cap > 0 && runtime_declared > 0
        && profile_cap == runtime_declared && actual_sysfs == profile_cap) {
        return "asb";
    }
    if (runtime_declared > 0 && actual_sysfs == runtime_declared
        && runtime_declared != profile_cap) {
        /* Our dynamic clamp is in effect and sysfs agrees with it. */
        return "asb_dynamic";
    }
    if (profile_cap > 0 && actual_sysfs == profile_cap
        && runtime_declared > 0 && runtime_declared != profile_cap) {
        /* sysfs matches profile, but runtime declared something else —
         * typical of thermal_overlay path writing one cap while the
         * underlying profile value is still what sticks at sysfs. */
