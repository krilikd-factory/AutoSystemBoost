...
static void session_plan_build(asb_fsm_t *fsm, int screen_on) {
    ...
    if (p == PROFILE_BATTERY && !screen_on && idle_band) {
         ...
         * was hot (>=75°C) and ended less than 120s ago */
        ...
        if (need_metrics) {
            ...
            if (!fsm.ses_intent_locked && fsm.ses_start_ts > 0) {
                ...
                if (ses_age >= 90 || have_thermal) {
                    ...
                    /* V33: benchmark false-positive guard — no real benchmark
                     * runs 15+ minutes. Downgrade to long_game/mixed and
                     * re-enable learning so pstats_performance gets data. */
                    if (fsm.ses_intent == INTENT_BENCHMARK &&
                        fsm.ses_intent_locked && ses_age >= 900) {
                        int old_intent = fsm.ses_intent;
                        fsm.ses_intent = have_gaming ? INTENT_LONG_GAME : INTENT_MIXED;
                    ...
                    if (g_ac_stage == AC_STAGE_BACKOFF && now_ac < g_ac_backoff_until) {
                        ...
                        if (max_gap > 500000 && metrics.therm.cpu_max_c < 95 && headroom_ok) {
                            ...
                            if (g_ac_stage == AC_STAGE_IDLE) {
                                 ...
                                 * budget=6 means 6 windows × ~3 writes = ~18 actual dual-writes max,
                ...
                if (++g_clamp_probe_skip >= probe_interval) {
                    ...
                    if (max_gap < 500000) {
                        ...
                        if (g_probe_good_hits >= 2) {
                            ...
                            session_plan_build(&fsm, metrics.misc.screen_on);
                            session_plan_apply_prearm(&fsm);
                            asb_log("clamp_probe: vendor clamp lifted (gap=%dkHz/%dkHz, 2+ good), plan rebuilt",
                                    gap0 / 1000, gap1 / 1000);
                        } else {
