...
static void pstats_save_one(const char *path, const asb_persistent_stats_t *ps) {
    FILE *f = fopen(path, "w");
    if (!f) return;
    fprintf(f, "{\"count\":%d,\"t2s\":%.1f,\"t2th\":%.1f,\"temp\":%.1f,\"gap\":%.0f,\"eff\":%.1f,\"deg\":%d,\"hot\":%d}",
            ps->session_count, ps->avg_time_to_first_sus,
            ps->avg_time_to_first_thermal, ps->avg_max_temp,
...
static void session_history_append_ex(const asb_fsm_t *fsm, const char *reason) {
    ...
    fprintf(wf,
        "{\"v\":4,\"ts\":\"%s\",\"profile\":\"%s\",\"mode\":\"%s\",\"end\":\"%s\","
        ...
        if (iq >= 70 && bat_total > 300) {
            ...
            if (bat_total > 300 && fsm->bat_wake_cycles > 0) {
                ...
                if (wake_rate > 1 && g_asb_cfg.bat_fast_idle_s > BAT_FAST_IDLE_FLOOR) {
                    ...
                    asb_log("self_tune: bat iq=%d wake_rate=%d -> bat_fast_idle %d->%d",
                            iq, wake_rate, old, g_asb_cfg.bat_fast_idle_s);
                    tuned++;
                }
            }
            if (bat_total > 120 && fsm->ses_time_heavy_sec > 0) {
                ...
                if (heavy_pct > 30 && g_asb_cfg.bat_heavy_load_enter < 8.0f) {
                    float old = g_asb_cfg.bat_heavy_load_enter;
                    g_asb_cfg.bat_heavy_load_enter += 0.5f;
                    if (g_asb_cfg.bat_heavy_load_enter > 8.0f)
                        g_asb_cfg.bat_heavy_load_enter = 8.0f;
                    asb_log("self_tune: bat iq=%d HEAVY=%d%% -> bat_heavy_load %.1f->%.1f",
                            iq, heavy_pct, old, g_asb_cfg.bat_heavy_load_enter);
                    tuned++;
                }
            }
            if (g_asb_cfg.bat_moderate_load_enter < 10.0f) {
                float old_m = g_asb_cfg.bat_moderate_load_enter;
                g_asb_cfg.bat_moderate_load_enter += 1.0f;
                asb_log("self_tune: bat iq=%d -> bat_moderate_load %.1f->%.1f",
    ...
    while (g_running) {
        ...
        static time_t g_last_heartbeat   = 0;
        {
            ...
            if (_now - g_last_heartbeat >= 900) {
                ...
                asb_log("heartbeat: state=%s profile=%d temp=%d load=%.1f gpu=%d bat=%d "
