...
static int battery_session_trust(const asb_fsm_t *fsm) {
    ...
    /* V38 final: partial-trust tier for mixed-day sessions. If iq is moderate
     * (10-19%), session is long (>=10min), wake frequency reasonable (<9/h) and
     * absolute wake count not extreme (<15), the session still carries usable
     * signal about daytime battery behavior. Feed learning at reduced weight
     * instead of discarding it like strict DIRTY does. */
    if (iq >= 10 && iq < 20 && dur >= 600 &&
        wph < 9.0f && fsm->bat_wake_cycles < 15)
        return BAT_TRUST_PARTIAL;
    if (iq < 20 && dur >= 300)
