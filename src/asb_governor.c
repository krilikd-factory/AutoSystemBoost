...
static void session_plan_build(asb_fsm_t *fsm, int screen_on) {
    ...
    if (p == PROFILE_BATTERY && !screen_on && idle_band) {
    ...
    } else if (p == PROFILE_BATTERY && (!screen_on || !heavy_band)) {
        ...
        fsm->plan.ac_budget   = 0;
    } else if (p == PROFILE_BATTERY) {
        fsm->plan.sensor_tier = 1;  /* REDUCED — battery active */
        fsm->plan.thermal_div = 1;
        fsm->plan.allow_hr    = 0;
        fsm->plan.ac_eligible = 0;
        fsm->plan.deep_sleep  = 0;
        ...
        fsm->plan.thermal_div = 1;
        fsm->plan.allow_hr    = 1;
        fsm->plan.ac_eligible = (p == PROFILE_PERFORMANCE || p == PROFILE_BALANCED);
        fsm->plan.deep_sleep  = 0;
...
int main(int argc, char **argv) {
    ...
    session_plan_apply_prearm(&fsm);
    asb_log("plan: sensor=%d thermal_div=%d hr=%d ac=%d deep=%d budget=%d prearm=%d",
            fsm.plan.sensor_tier, fsm.plan.thermal_div,
            fsm.plan.allow_hr, fsm.plan.ac_eligible, fsm.plan.deep_sleep,
            fsm.plan.ac_budget, fsm.plan.ac_prearm);
    ...
    while (g_running) {
        ...
        if (need_metrics) {
            /* V31: sensor scheduler reads from session plan (no per-tick conditionals) */
                    ...
                    if (g_ac_stage == AC_STAGE_BACKOFF && now_ac < g_ac_backoff_until) {
                        ...
                        if (max_gap > 500000 && metrics.therm.cpu_max_c < 95 && headroom_ok) {
                            ...
                            if (g_ac_stage == AC_STAGE_IDLE) {
                                /* V31: prearm skips detection delay — enter BURST immediately */
                                if (fsm.plan.ac_prearm) {
                                    g_ac_stage = AC_STAGE_BURST;
                                    g_ac_burst_count = 0;
                                    g_ac_fails = 0;
                                    asb_log("anti_clamp: prearm -> immediate BURST (streak=%d)",
                    ...
                    if (vendor_clamping) {
                        ...
                        if (max_delta < 100000) {
                            ...
                            if (g_ac_fails >= 5) {
                                ...
                                g_ac_fails = 0;
                                asb_log("anti_clamp: backoff 30s (5 ineffective, actual=[%d,%d])",
                                        post_p0, post_p1);
                            }
                        } else {
                            g_ac_fails = 0;
                        ...
                        /* V31: consume budget on each aggressive dual-write */
                        fsm.plan.ac_used++;
                        if (fsm.plan.ac_used >= fsm.plan.ac_budget) {
                            asb_log("anti_clamp: budget exhausted (%d/%d), disabling for session",
