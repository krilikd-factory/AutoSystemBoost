...
#define ACTION_WASTE_THRESHOLD 5
...
#define QUIET_NIGHT_ENTER_TICKS  60       /* ~10min at TIMER_DEEP_S=10s */
#define QUIET_NIGHT_FAST_TICKS   30       /* ~5min with clean-night reward */
#define TIMER_ULTRA_DEEP_S       20       /* tick interval in ultra-quiet mode */
...
static int classify_environment(const asb_fsm_t *fsm) {
    ...
    if (wph > 12.0f || iq < 10 || (radio_noisy && iq < 20))
        return ENV_HOSTILE;
    if (wph > 6.0f || iq < 25 || radio_noisy)
        return ENV_NOISY;
    return ENV_QUIET;
}
    ...
    while (g_running) {
        ...
        static time_t g_last_heartbeat   = 0;
        {
            ...
            if (_now - g_last_heartbeat >= 900) {
                ...
                /* V34: action waste natural decay -- prevents sticky high waste
                 * after a bad phase has passed. ~75min to fully recover from threshold. */
                if (g_action_waste > 0) g_action_waste--;
            }
        }
        ...
        if (need_metrics) {
            ...
            if (fsm.state == ASB_STATE_DEEP_IDLE &&
                ...
                int threshold = g_last_bat_clean_night
                                ? QUIET_NIGHT_FAST_TICKS   /* 5min with reward */
                                : QUIET_NIGHT_ENTER_TICKS; /* 10min normal */
                ...
                if (g_quiet_night_active) {
                    g_quiet_noise_ticks++;
                    if (g_quiet_noise_ticks >= 3 || metrics.misc.screen_on) {
            ...
            int clamp_economy = (fsm.clamp_hold && g_clamp_hold_since > 0 &&
                                 (time(NULL) - g_clamp_hold_since) > 120 &&
            ...
            if (clamp_economy) {
                ...
                if (g_clamp_thermal_skip % 3 != 0) need_thermal = 0;
            }
            ...
            if ((fsm.state == ASB_STATE_HEAVY || fsm.state == ASB_STATE_GAMING
                ...
                if (g_action_waste >= ACTION_WASTE_THRESHOLD)
            ...
            }
            {
                ...
                if (g_quiet_night_active) want = TIMER_ULTRA_DEEP_S;
