...
static const char *intent_names[] = {"unknown","benchmark","long_game","idle","mixed"};
...
static void session_history_append_ex(const asb_fsm_t *fsm, const char *reason) {
    ...
    fprintf(wf,
        "{\"v\":3,\"ts\":\"%s\",\"profile\":\"%s\",\"mode\":\"%s\",\"end\":\"%s\","
        ...
        "\"idle_q\":%d,\"cap_eff\":%d,\"dur\":%ld,\"intent\":\"%s\"}\n",
        ...
        (fsm->ses_intent >= 0 && fsm->ses_intent <= 4)
        ...
        if (need_metrics) {
            ...
            if (!fsm.ses_intent_locked && fsm.ses_start_ts > 0) {
                ...
                if (ses_age >= 90 || have_thermal) {
                    ...
                    if (fsm_profile_is_battery) {
                        fsm.ses_intent = (fsm.bat_time_deep_idle_sec > 60)
                                         ? INTENT_IDLE : INTENT_MIXED;
                    ...
                    if (fsm.ses_intent != INTENT_UNKNOWN) {
                        ...
                        if (fsm.profile_idx == PROFILE_PERFORMANCE &&
                            ...
                            if (fsm.ses_intent == INTENT_BENCHMARK && have_thermal) {
                                /* Benchmark already hitting thermal: degrade faster.
                                 * Reduce the time gate for Path 2 to 60s. */
                                /* (params already in auto, degrade will fire soon) */
                                asb_log("intent: benchmark+thermal -> degrade imminent");
                            } else if (fsm.ses_intent == INTENT_LONG_GAME) {
                                /* Long game: raise sustained_level for better
                                 * throughput, lengthen cooldowns for stability */
