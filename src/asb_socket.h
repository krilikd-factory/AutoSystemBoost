#pragma once
/*
 * asb_socket.h — Unix domain socket для управления daemon
 *
 * action.sh и service.sh могут слать команды:
 *   "profile:battery"    — сменить профиль
 *   "profile:balanced"
 *   "profile:performance"
 *   "status"             — получить JSON статус
 *   "reload"             — перечитать конфиг
 *   "quit"               — остановить daemon
 *
 * Ответ на "status":
 *   {"state":"MODERATE","profile":"balanced","mA":145,"gpu":23,
 *    "cpu_max":[1324800,2438400,3072000],"thermal":0,"learn":"LIGHT"}
 *
 * Сокет: /dev/.asb/ctl.sock (tmpfs, автоматически исчезает при ребуте)
 */
...
/* Создаёт listening socket, возвращает fd или -1 */
static int asb_sock_create(void) {
    ...
    unlink(ASB_SOCK_PATH); /* удаляем старый сокет если был */
...
/* Читает команду (non-blocking). Возвращает длину или <=0. */
...
/* Команда из action.sh (одноразовая отправка) */
static int asb_sock_send_cmd(const char *cmd, char *reply, int reply_len) {
    int fd = socket(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0);
    if (fd < 0) return -1;

    /* Bind к временному адресу для получения ответа */
    ...
    if (reply && reply_len > 0) {
        struct timeval tv = {1, 0}; /* 1с таймаут */
