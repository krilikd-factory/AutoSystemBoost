...
static int writer_apply_caps(const asb_profile_caps_t *caps, int force, asb_state_t state, int thermal_cap) {
    ...
    /* CPU min freq
     * Защита от min > max:
     * 1. При thermal_cap=1 не пишем вообще (наш threshold 65°C, но vendor
     *    thermal может работать раньше).
     * 2. Даже при thermal_cap=0: перед записью читаем текущий scaling_max_freq
     *    и ограничиваем min значением не выше текущего max. Vendor thermal
     *    может тихо снижать max не меняя нашего thermal_cap.               */
    if (!thermal_cap) {
        for (int i = 0; i < 3; i++) {
            if (!g_cpu_min_paths[i][0]) continue;
            int want_min = caps->cpu_min[i];
            if (want_min <= 0) continue;
            /* Читаем текущий max из sysfs (не из кэша — vendor мог изменить) */
            int cur_max = sysfs_read_int(g_cpu_max_paths[i], 0);
            if (cur_max > 0 && want_min > cur_max)
                want_min = cur_max; /* clamp: не позволяем min > текущий max */
