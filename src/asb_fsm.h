...
static const asb_profile_bounds_t g_profile_bounds[3] = {
    /* V39r3: ALL THREE profile bounds realigned with their .sh scripts.
     *
     ...
     *
     * V39r6: caps recalibrated for Snapdragon 8 Elite Gen 5 (4.6GHz P-cluster, 1200MHz GPU).
    ...
    {
        .floor = {
            .cpu_max    = { 1132800, 1113600, 0 },   /* = CPU_CAP, raised from 614/921 */
            .cpu_min    = {  307200,  614400, 0 },   /* = CPU_MIN */
            .gpu_max_pct = 30, .gpu_min_pct = 0,     /* raised from 5 — UI composer at 144Hz */
            .ravg_ticks = 8, .idle_enough = 95,
            .uclamp_top_max = 35, .uclamp_bg_max = 25  /* raised from 10/5 — UI/GC must work */
        },
        .ceil = {
            .cpu_max    = { 1804800, 2400000, 0 },   /* = CPU_MAX, raised from 1132/998 */
            .cpu_min    = { 1132800, 1113600, 0 },   /* = CPU_CAP */
            .gpu_max_pct = 55, .gpu_min_pct = 0,     /* = GPU_MAX_PCT, raised from 15 */
            .ravg_ticks = 8, .idle_enough = 95,
            .uclamp_top_max = 65, .uclamp_bg_max = 40  /* raised from 25/10 */
        }
    },
     ...
     *
     * Before V39r3: FSM wrote DEEP_IDLE={1728, 2266}/HEAVY={2461, 3261} — floor
     ...
     */
    {
        .floor = {
            ...
            .gpu_max_pct = 42, .gpu_min_pct = 0,     /* half of GPU_MAX (85/2≈42) */
        ...
        .ceil = {
            ...
            .gpu_max_pct = 85, .gpu_min_pct = 0,     /* = GPU_MAX_PCT from .sh */
            .ravg_ticks = 3, .idle_enough = 45,
            .uclamp_top_max = 85, .uclamp_bg_max = 35  /* V39r6: 80→85 for UI burst headroom */
        }
    },
     ...
     *
     * Before V39r3: FSM floor.BIG=3072 was ABOVE CPU_CAP_BIG=2976 (the V38
...
static int fsm_update(asb_fsm_t *fsm, const asb_metrics_t *m) {
    ...
    if (!m->misc.screen_on && fsm->state != ASB_STATE_DEEP_IDLE) {
         ...
         * cap to MODERATE to prevent unnecessary heating. The user chose battery
         * profile for efficiency, not for pushing heavy states on warm silicon. */
