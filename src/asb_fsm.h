...
static const asb_profile_bounds_t g_profile_bounds[3] = {
    ...
    {
        .floor = {
            .cpu_max    = { 1132800, 1113600, 0 },   /* = CPU_CAP */
            .cpu_min    = {  307200,  614400, 0 },   /* = CPU_MIN */
            /* GPU floor raised 30→45%: V40 deploy revealed vendor PowerHAL on
             * SM8850 clamps GPU to 160MHz floor when it sees low FSM cpu_max
             * + LIGHT_IDLE state. UI scrolling happens in LIGHT_IDLE → was
             * stutter. Raising GPU max floor to 45% (540MHz target) breaks
             * that vendor heuristic. Also raising gpu_min_pct=0→18% so
             * min_pwrlevel write is non-trivial — gpu_min_pct=0 caused us to
             * write min_pwrlevel=17 (lowest level), which vendor read as
             * "user wants minimum freq" and clamped max to 17 too. 18% =
             * 216MHz target → min_pwrlevel=15 (222MHz), gives vendor a
             * meaningful lower bound to respect. */
            .gpu_max_pct = 45, .gpu_min_pct = 18,
            .ravg_ticks = 8, .idle_enough = 95,
            .uclamp_top_max = 35, .uclamp_bg_max = 25
        },
        .ceil = {
            .cpu_max    = { 1804800, 2400000, 0 },   /* = CPU_MAX */
            .cpu_min    = { 1132800, 1113600, 0 },   /* = CPU_CAP */
            .gpu_max_pct = 55, .gpu_min_pct = 18,    /* V40: same min as floor */
            .ravg_ticks = 8, .idle_enough = 95,
            .uclamp_top_max = 65, .uclamp_bg_max = 40
        }
    },
    ...
    {
        .floor = {
            .cpu_max    = { 1190400, 1881600, 0 },   /* = CPU_CAP */
            .cpu_min    = {  787200,  883200, 0 },   /* = CPU_MIN */
            .gpu_max_pct = 42, .gpu_min_pct = 18,    /* V40: gpu_min raised 0→18 */
            .ravg_ticks = 4, .idle_enough = 50,
            .uclamp_top_max = 50, .uclamp_bg_max = 25
        },
        .ceil = {
            .cpu_max    = { 3302400, 3974400, 0 },   /* = CPU_MAX */
            .cpu_min    = { 1190400, 1881600, 0 },   /* = CPU_CAP */
            .gpu_max_pct = 85, .gpu_min_pct = 18,    /* V40: same as floor */
            .ravg_ticks = 3, .idle_enough = 45,
            .uclamp_top_max = 85, .uclamp_bg_max = 35  /* V39: 80→85 for UI burst headroom */
        }
    },

    /* PROFILE_PERFORMANCE — from performance.sh:
     * CPU_MIN={1190400, 1113600}  CPU_CAP={3072000, 2976000}  CPU_MAX={3628800, 4608000}  GPU_MAX=84%
     *
     * Before V39: FSM floor.BIG=3072 was ABOVE CPU_CAP_BIG=2976 (the V38
     * cooling trim), so the trim never actually took effect in-state. Also
     * ceil.gpu_max_pct was 100%, bypassing the .sh GPU_MAX=84% cap.
     *
     * Note floor.LITTLE goes UP from 2265600 to 3072000 (= CPU_CAP_LITTLE).
     * This is intentional: performance profile's DEEP_IDLE can run at the
     * profile's nominal cap. Users who want cooler idle on performance can
     * lower CPU_CAP_LITTLE in performance.sh.
     */
    {
        .floor = {
            .cpu_max    = { 3072000, 2976000, 0 },   /* = CPU_CAP (respects V38 cooling trim) */
            .cpu_min    = { 1190400, 1113600, 0 },   /* = CPU_MIN */
            .gpu_max_pct = 42, .gpu_min_pct = 10,    /* half of GPU_MAX (84/2=42) */
            .ravg_ticks = 2, .idle_enough = 10,
            .uclamp_top_max = 80, .uclamp_bg_max = 55
        },
        .ceil = {
            .cpu_max    = { 3628800, 4608000, 0 },   /* = CPU_MAX */
            .cpu_min    = { 3072000, 2976000, 0 },   /* = CPU_CAP (keep min high during GAMING) */
            .gpu_max_pct = 84, .gpu_min_pct = 30,    /* = GPU_MAX_PCT from .sh (was 100) */
...
static int fsm_update(asb_fsm_t *fsm, const asb_metrics_t *m) {
    ...
    if (!m->misc.screen_on && fsm->state != ASB_STATE_DEEP_IDLE) {
        ...
        if (fsm_profile_is_battery && m->misc.screen_on &&
            ...
            /* V40-diag: log first time this fires per minute for visibility */
