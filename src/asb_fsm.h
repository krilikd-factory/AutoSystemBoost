...
static asb_state_t fsm_desired(const asb_metrics_t *m) {
    ...
    if (m->therm.throttling &&
        (m->gpu.load_pct >= g_asb_cfg.sustained_gpu_min ||
         m->cpu.load1 >= g_asb_cfg.sustained_load_min ||
         m->therm.cpu_max_c >= g_asb_cfg.sustained_temp_enter))
        return ASB_STATE_SUSTAINED;
...
static int fsm_update(asb_fsm_t *fsm, const asb_metrics_t *m) {
    ...
    /* ── Screen ON из DEEP_IDLE: немедленный подъём ─────────── */
        ...
        if (m->therm.throttling && desired >= ASB_STATE_HEAVY) {
