...
static int fsm_update(asb_fsm_t *fsm, const asb_metrics_t *m) {
    ...
    /* ── Screen ON из DEEP_IDLE: немедленный подъём ─────────── */
            ...
            desired >= ASB_STATE_HEAVY)
        {
            ...
            fsm->sustained_reason = 0; /* thermal */
        }
            ...
            g_asb_cfg.gaming_gap_thresh > 0)
        {
                ...
                desired >= ASB_STATE_HEAVY)
            {
                desired = ASB_STATE_SUSTAINED;
                gap_to_sustained = 1;
                fsm->gaming_gap_ticks_count = 0; /* сброс после промоции */
                fsm->sustained_reason = 1; /* gaming_unreachable */
            }
        ...
        if (!thermal_to_sustained && !gap_to_sustained &&
            ...
            fsm->prev_state == ASB_STATE_SUSTAINED &&
            fsm->gaming_retry_until > 0)
        {
            time_t now_t = time(NULL);
            if (now_t < fsm->gaming_retry_until) {
                desired = ASB_STATE_HEAVY; /* cooldown активен */
            } else {
                fsm->gaming_retry_until = 0; /* cooldown истёк */
            }
        }
        /* Устанавливаем cooldown при выходе из SUSTAINED */
        if (fsm->prev_state == ASB_STATE_SUSTAINED &&
            fsm->state_changed &&
            fsm->state != ASB_STATE_SUSTAINED &&
            g_asb_cfg.gaming_retry_cooldown_s > 0)
        {
            fsm->gaming_retry_until = time(NULL) + g_asb_cfg.gaming_retry_cooldown_s;
        }
        ...
        if (fsm->pending_ticks >= window && desired != fsm->state) {
            int can_leave = 1;
            if (desired < fsm->state) {
                int min_dwell = fsm_min_dwell_for_state(fsm->state);
                if (min_dwell > 0 && fsm_elapsed_sec(fsm) < min_dwell)
                    can_leave = 0;
                /* Температурный гистерезис выхода из SUSTAINED:
                 * не выходим пока temp > sustained_temp_exit.
                 * Предотвращает cycling при температуре 57-64°C. */
