...
typedef struct {
    ...
    /* V28r2: Session intent -- classified after first 90s of data.
     * Used by governor to adjust burst/cooldown params mid-session. */
    int             ses_intent;  /* 0=unknown, 1=benchmark, 2=long_gaming, 3=idle, 4=mixed, 5=sleep_idle */
    int             ses_intent_locked; /* 1 = intent classified, no more changes */
    long            ses_degrade_at_age; /* V27: seconds from session start to auto-degrade (0=not degraded) */
} asb_fsm_t;
...
/* V28r3: Flush accumulated time in current state into counters.
 * Fixes bug where bat_time_deep_idle_sec stayed at 30s after 8 hours
 * of DEEP_IDLE because time was only accumulated on state *transitions*.
 * Call before session_history_append, idle_boundary, and shutdown. */
static inline void fsm_flush_state_time(asb_fsm_t *fsm) {
    ...
    /* Reset anchor so next flush/transition doesn't double-count */
