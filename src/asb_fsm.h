...
typedef struct {
    ...
    long            ses_gap_p1_sum;
    int             ses_gap_samples;         /* знаменатель для avg */
    int             ses_max_gap_p0;
    int             ses_max_gap_p1;
    int             ses_max_temp;            /* максимальная температура за сессию */

    struct timespec ses_state_enter;         /* когда вошли в текущий state (для накопления времён) */
} asb_fsm_t;
...
static void fsm_init(asb_fsm_t *fsm, int profile_idx) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &fsm->ses_state_enter); /* session timing start */
    /* Начальные caps */
