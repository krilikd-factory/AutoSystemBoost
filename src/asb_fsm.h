...
static const asb_profile_bounds_t g_profile_bounds[3] = {
    /* V39r3: BATTERY bounds realigned with battery.sh profile script.
     *
     * Previously g_profile_bounds[BATTERY] carried hardcoded caps 1190/1344
     * (floor) and 1613/2266 (ceil) which were ~2x higher than what battery.sh
     * intended (profile_cap 614/922, profile_max 1132/998). Probe on real
     * device confirmed LIGHT_IDLE wrote 1253/1482 kHz — exactly matching
     * lerp(hardcoded_floor, hardcoded_ceil, 0.15). Shell reconcile's writes
     * of 614/922 were immediately overwritten by FSM's interpolated caps.
     *
     * V39r3 bounds match battery.sh directly:
     *   floor (DEEP_IDLE, t=0)  = minimum acceptable cap in idle
     *   ceil  (GAMING,   t=1.0) = absolute max allowed on battery profile
     *
     * LIGHT_IDLE (t=0.15) now resolves to ~691/672 kHz — close to the
     * profile base that users see in battery.sh, not 2x higher.
     */
    {
        .floor = {
            .cpu_max    = {  614400,  614400, 0 },
            .cpu_min    = {  307200,  614400, 0 },
            .gpu_max_pct = 15, .gpu_min_pct = 0,
            .ravg_ticks = 8, .idle_enough = 95,
            .uclamp_top_max = 10, .uclamp_bg_max = 5
        },
        .ceil = {
            .cpu_max    = { 1132800,  998400, 0 },
