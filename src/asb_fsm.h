...
static asb_state_t fsm_desired(const asb_metrics_t *m) {
    ...
    /* GAMING: GPU сильно занят + большое потребление */
    if (m->gpu.load_pct >= 65 && m->bat.current_ma >= 350)
        return ASB_STATE_GAMING;

    /* HEAVY: видео/карты/рендеринг */
    if (m->gpu.load_pct >= 35 && m->bat.current_ma >= 180)
        return ASB_STATE_HEAVY;

    /* MODERATE: активное использование */
    if (m->cpu.load1 >= 1.5f || m->bat.current_ma >= 150)
