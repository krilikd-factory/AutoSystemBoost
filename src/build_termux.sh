...
#
# РЕЗУЛЬТАТ:
#   ../bin/asb_governor  (ARM64 PIE бинарник, ~120KB stripped)
...
echo "[ASB] Compiling asb_governor..."
...
clang $CFLAGS \
    ...
    $LDFLAGS \
    -o "$OUTDIR/asb_governor"

# Стриппинг debug symbols (уменьшает размер)
if command -v strip >/dev/null 2>&1; then
    strip "$OUTDIR/asb_governor"
fi

SIZE=$(wc -c < "$OUTDIR/asb_governor")
echo "[ASB] Binary size: ${SIZE} bytes"

# Проверяем что бинарник запускается
"$OUTDIR/asb_governor" status 2>/dev/null || true

echo "[ASB] Build successful: $OUTDIR/asb_governor"
echo ""
echo "Следующий шаг:"
echo "  cp $OUTDIR/asb_governor /data/adb/modules/AutoSystemBoost/bin/"
echo "  chmod 755 /data/adb/modules/AutoSystemBoost/bin/asb_governor"
