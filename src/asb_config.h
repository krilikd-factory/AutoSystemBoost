...
static inline void asb_config_apply_highload_mode(asb_runtime_config_t *c) {
    if (c->highload_mode == 1) {        /* BURST: fast retry into GAMING, no cycling */
        c->gaming_gap_ticks             = 3;    /* быстрее промоция по gap */
        c->gaming_retry_cooldown_s      = 20;   /* быстрее retry после SUSTAINED */
        c->gaming_retry_temp_max        = 55;   /* более тёплый retry */
        c->sustained_level              = 0.80f; /* не 0.85: меньше thermal pressure */
        c->sustained_reentry_cooldown_s = 20;   /* не 10: убираем cycling */
