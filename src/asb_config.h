...
typedef struct {
    ...
    /* auto-mode thresholds: когда degradировать burst→stable */
    int   auto_degrade_gap_thresh;   /* avg_gap_p0 > X → gaming caps недостижимы */
    int   auto_degrade_sus_ratio;    /* sus_entries / gaming_entries >= X → stable */
    int   auto_degrade_thermal_pct;  /* % времени в SUSTAINED от (heavy+gaming+sustained) → stable без gaming */

    /* Battery profile tuning */
    int   bat_fast_idle_s;       /* сек до перехода в DEEP_IDLE при battery профиле (0=off) */
    int   bat_light_idle_gpu;    /* GPU % cap в LIGHT_IDLE при battery */
    int   bat_suppress_gaming;   /* 1 = не входить в GAMING при battery профиле */
...
static inline void asb_config_defaults(asb_runtime_config_t *c) {
    ...
    c->highload_mode = 0;
    c->auto_degrade_gap_thresh  = 800000; /* kHz: avg gap > 800MHz → gaming недостижим */
    c->auto_degrade_sus_ratio   = 4;
    c->auto_degrade_thermal_pct = 60;

    /* Battery profile */
    c->bat_fast_idle_s     = 15; /* battery: через 15с без активности → DEEP_IDLE */
    c->bat_light_idle_gpu  = 10; /* battery: GPU max 10% в LIGHT_IDLE */
    c->bat_suppress_gaming = 1;  /* battery: GAMING заблокирован */ /* 60% тяжёлого времени в SUSTAINED → деградация */
}
...
/* При старте/reload применяет базовые параметры режима.
 * auto=3 стартует как burst; деградация происходит в рантайме. */
...
/* Применяет stable-параметры поверх текущего конфига (auto degrade).
 * Вызывается однократно за сессию когда governor решает что burst бессмысленен. */
...
/* AUTO degrade: burst→stable при плохой игровой достижимости.
 * Оба условия должны выполняться:
 *  1. avg_gap_p0 > auto_degrade_gap_thresh  (caps стабильно недостижимы)
 *  2. sus_entries >= gaming_entries * ratio  (GAMING редок, SUSTAINED доминирует)
 * Минимум 2 gaming_entries нужно для статистики. */
        ...
        int already_degraded)
{
    ...
    /* Путь 1: gaming entries есть, caps недостижимы */
    ...
    /* Путь 2: thermal pressure — даже без gaming entries.
     * Если > auto_degrade_thermal_pct% тяжёлого времени проведено в SUSTAINED
     * и накоплено достаточно данных (>120s) — burst бессмысленен.         */
