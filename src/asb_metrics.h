...
static void thermal_discover(void) {
    ...
    for (int z = 0; z < THERMAL_MAX_ZONES; z++) {
        ...
        /* Приоритет: cpullc (cluster avg) > cpu-1-1 (per-core hotspot)
         * cpullc-0-0 на SD8 Elite = zone0, стабильнее per-core на 15-20°C */
        if (strstr(type, "cpullc")) {
            /* Cluster-level temperature — лучший выбор */
            if (g_thermal_cpu_zone < 0) g_thermal_cpu_zone = z;
        } else if (strstr(type, "cpu-1-1") || strstr(type, "cpuss-0") ||
                   strstr(type, "cpu-1-4")) {
            /* Per-core fallback если cpullc не найден */
            if (g_thermal_cpu_zone < 0) g_thermal_cpu_zone = z;
        }
