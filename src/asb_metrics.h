...
static void thermal_discover(void) {
    ...
    for (int z = 0; z < THERMAL_MAX_ZONES; z++) {
        ...
        /* V37-r6: OP15-accurate CPU hotspot priority.
         ...
         * socd returns RAW degrees (74), not millidegrees. Everything else is millidegrees.
         */
        int cpu_prio = -1;
        ...
        /* V37-r6: OP15-accurate skin/surface priority.
         * Confirmed on OnePlus 15:
         *   - shell_front / shell_frame / shell_back: direct surface sensors (~30C idle)
         *   - sys-therm-6: hottest general skin-adjacent zone (~40C idle) -- ghost hotspot
         *   - sys-therm-0/2/3/4/5/7/9/12: other general thermistors
         *   - board_temp: PCB reference
         * We want the HOTTEST of these as the "skin feeling" proxy, so sys-therm-6
         * gets priority 1 since that's where real surface heat shows up on OP15.
         */
        int skin_prio = -1;
        if (strcmp(type, "sys-therm-6") == 0)
            skin_prio = 1;  /* OP15: hottest general skin-adjacent zone */
        else if (strcmp(type, "shell_frame") == 0)
            skin_prio = 2;
            ...
            skin_prio = 5;  /* any other sys-therm-N as fallback */
        else if (strstr(type, "skin-virt") || strstr(type, "skin-msm"))
            skin_prio = 6;  /* legacy names for other SoCs */
        else if (strstr(type, "back-therm") || strcmp(type, "skin") == 0)
            skin_prio = 7;
...
static void metrics_read_thermal(asb_thermal_t *t, int need_headroom) {
    ...
    /* V37-r6: if skin or cpu zone wasn't found at startup (validate failed on a
     * transient read), retry every 60 seconds. Cheap -- just iterates 98 sysfs
     * files. Only triggers while a zone is still -1. */
    if (g_thermal_skin_zone < 0 || g_thermal_cpu_zone < 0) {
    ...
    if (g_thermal_skin_zone >= 0) {
        ...
        /* Dual-format: most zones return millidegrees, some return raw Celsius.
         * Normalize identically to cpu_max_c handling. */
        if (sv > 0) t->skin_temp_c = (sv > 200) ? (sv / 1000) : sv;
        else t->skin_temp_c = 0;
    }
