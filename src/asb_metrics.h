...
/* V34: radio-aware -- mobile data paths */
#define PATH_RMNET_TX       "/sys/class/net/rmnet_data0/statistics/tx_bytes"
#define PATH_RMNET_RX       "/sys/class/net/rmnet_data0/statistics/rx_bytes"
...
static void thermal_discover(void) {
    ...
    for (int z = 0; z < THERMAL_MAX_ZONES; z++) {
        ...
        /* CPU zone priority: lower = better */
        int cpu_prio = -1;
        if (strstr(type, "cpu-1-1") || strstr(type, "cpuss-0"))
            cpu_prio = 1;  /* big core cluster -- best */
        else if (strstr(type, "cpu-1-4") || strstr(type, "cpuss-1"))
            cpu_prio = 2;  /* prime core */
        else if (strstr(type, "socd"))
            cpu_prio = 3;  /* SoC die -- stable, slightly delayed */
        else if (strstr(type, "cpullc"))
            cpu_prio = 8;  /* little core -- unstable, last resort */

        if (cpu_prio > 0 && cpu_prio < best_cpu_prio) {
            /* V34: validate before accepting */
...
static void metrics_read_thermal(asb_thermal_t *t, int need_headroom) {
     ...
     * Format: "0:FREQ 1:FREQ 2:FREQ ... 6:FREQ 7:FREQ\n"
     * These reflect thermal throttling at kernel level -- higher priority than
     * scaling_max_freq. Gives ~1-2s earlier warning than thermal zone temp. */
    if (need_headroom) {
        char buf[256];
        ...
        if (fd >= 0) {
            ...
            if (n > 0) {
                buf[n] = '\0';
                /* Parse "0:FREQ" for policy0 and "6:FREQ" for policy6 */
                char *p = buf;
                while (*p) {
                    int cpu = -1, freq = 0;
                    ...
                    /* advance to next token -- skip current token */
                    while (*p && *p != ' ' && *p != '\n' && *p != '\t') p++;
                    /* skip all whitespace including newlines */
                    while (*p == ' ' || *p == '\n' || *p == '\t' || *p == '\r') p++;
                }
                /* Compute headroom: what % of max hardware freq is available
                 * policy0 max = 3628800, policy6 max = 4608000
                 * Use policy0 as primary (6 cores, most workload impact) */
                if (t->perf_cap_p0 > 0) {
                    ...
                    /* Two-level headroom model:
                     * <70% -> set throttling flag (advisory -- used for logging and
                     *         existing temp-based sustained entry path)
                     * <50% -> separate FSM path in asb_fsm.h forces SUSTAINED
                     *         (actionable -- burst is futile at this point)
                     * These are intentionally different thresholds for different purposes. */
                    if (t->headroom_pct < 70) t->throttling = 1;
                    t->headroom_valid = 1;
                }
            }
        }
...
static void metrics_read_network(asb_misc_t *m, const struct timespec *now) {
    ...
    long mtx = sysfs_read_long(PATH_RMNET_TX, 0);
    long mrx = sysfs_read_long(PATH_RMNET_RX, 0);
