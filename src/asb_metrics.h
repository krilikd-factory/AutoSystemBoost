...
typedef struct {
     ...
     * Values: "ok", "no_zone", "read_fail", "stale", "raw_too_low", "init" */
...
static void thermal_discover(void) {
    ...
    for (int z = 0; z < THERMAL_MAX_ZONES; z++) {
        ...
        if (cpu_prio > 0 && cpu_prio < best_cpu_prio) {
            ...
            /* V37.1 socd-specific sanity gate: a "valid" reading of <=10C
             * is implausible while CPU is actually drawing power. Reject. */
    ...
    if (g_thermal_cpu_zone >= 0) {
        ...
        if (v > 0) {
            ...
            if (c_now > 0 && c_now <= 10) {
                ...
                t->cpu_max_c = c_now;
                t->temp_valid = 1;
                snprintf(t->temp_invalid_reason, sizeof(t->temp_invalid_reason), "ok");
