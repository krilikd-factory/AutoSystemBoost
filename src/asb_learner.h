 ...
 *
 * Algorithm: Exponential Moving Average by hour-of-day × day-of-week.
 * 24 hours × 7 days = 168 slots.
 * Each slot stores: avg drain (mA), avg screen-on time.
 *
 * learn.bin: 168 × 8 bytes = 1344 bytes. Stored in tmpfs.
 *
 * EMA coefficient α = 0.15:
 *   - New data influences gradually
 *   - Memory ≈ 1/α = ~7 measurements = ~7 days
...
typedef struct {
    ...
    float _pad;
} asb_slot_t;              /* 16 bytes × 168 = 2688 bytes */
