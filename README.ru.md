...
| 🥇 **Полная поддержка — проверено в поле** | OnePlus Ace 5 · 13R (PKG110 / CPH2691) | Snapdragon 8 Gen 3 (SM8650) | `giuliac` · `giulia` |
| ✅ **Device-native** | OnePlus 15R, 13s / 13T, 12R, 11 / 11R, Open, Ace 6T, Ace / Nord / Pad | разные | — |
...
**Ace 6 и Ace 5 проходят тот же самый пайплайн** — те же стадии clone-and-patch
для аудио, камеры, медиа, GPS, perf и Wi-Fi, `asbdiag` PASS, чистый первый бут.
Ничего не пропускается, это **не** урезанный режим совместимости. Отличается лишь
доставка: их `/odm` аудио и медиа приходят через **runtime `mount --bind`** (клон в
`/data/adb/asb/odm_patched/`, патч, SELinux-контекст снимается с живой цели,
применение в `post-fs-data` до зигота), они работают под более строгим
**1-strike boot fuse** вместо 3-strike, а device-adaptive CPU bounds включены по
умолчанию на Ace 5 (SM8650-lean) и остаются опциональными через WebUI на Ace 6.

**Все остальные OnePlus** получают тот же device-native `/vendor`-оверлей под тем же
1-strike fuse: одна неудачная загрузка вырезает сгенерированный оверлей *до*
монтирования модуля, и устройство поднимается в режиме governor-only. Сам раздел
`/odm` никогда не изменяется, и поверх него никогда не графтится каталог.
