...
if [ "$ASB_BT" = "true" ]; then
  ...
  ui_print " "
  # Camera belongs with the other tweak categories, after Wi-Fi.
  #
  # It briefly sat near the end of the installer, printed after CONFIG and outside the
  # category group - a side effect of moving the block to reach a flag that was itself
  # defined too late. With the flags resolved up front there is no reason for it to be
  # anywhere but here.
...
asb_preserve_user_config() {
  ...
  #  - were never put on it. Their values were dropped on every update
...
# Call recording was removed; undo it for anyone who had it on.
#
# The feature is gone from this build, so nothing would ever restore what it changed:
# VoiceScribe's patched preferences (kept as .asb.bak), the microphone and storage
# permissions it was granted, and the staged XML payloads. The PREVIOUS module copy is
# still on disk at this point in the install, and it knows how to undo all of that -
# so ask it to, once, before it is replaced.
...
rm -f /data/adb/asb/callrec_line_manifest.txt /data/adb/asb/callrec_line.active \
      ...
      /data/adb/asb/callrec_boot_pending /data/adb/asb/callrec_blocked \
      /data/adb/asb/callrec_line_state /data/adb/asb/callrec_empty.pcm 2>/dev/null
